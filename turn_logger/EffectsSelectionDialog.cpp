    ...
    plane_idx(plane_idx)
{
    ...
    for (int i=0; i<plane_model->rowCount(plane_idx); ++i) {
        ...
        if (crew_idx.data(Qt::UserRole).toInt() == BaseItem::Plane_Crew_Item_Type) {
            ...
            for (int j=0; j<plane_model->rowCount(crew_idx); ++j) {
                ...
                if (crew_child_idx.data(Qt::UserRole).toInt() == BaseItem::Plane_Armaments_Item_Type &&
                ...
                else if (crew_child_idx.data(Qt::UserRole).toInt() == BaseItem::Plane_Armaments_Item_Type) {
...
void EffectsSelectionDialog::accept()
{
    if (ui->rudderJamBox->isChecked()) {
        plane_model->setData(plane_idx.sibling(plane_idx.row(), PlaneItemOld::Rudder_State), ui->jammed_left_radio->isChecked() ?
                                 PlaneItemOld::Rudder_Jammed_Left : PlaneItemOld::Rudder_Jammed_Right);
        plane_model->setData(plane_idx.sibling(plane_idx.row(), PlaneItemOld::Rudder_Jam_Duration), ui->jam_duration->value());
    }
